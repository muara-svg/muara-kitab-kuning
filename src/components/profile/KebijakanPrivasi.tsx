...
export default function KebijakanPrivasi({ onClose }: KebijakanPrivasiProps) {
  ...
  const hasContent = activeConfig.privacyPolicy?.content && activeConfig.privacyPolicy.content.trim().length > 0;
  const displayTitle = hasContent ? activeConfig.privacyPolicy.title : 'Kebijakan Privasi';
  const displayContent = hasContent ? activeConfig.privacyPolicy.content : '';
    ...
    >
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-6 py-8 md:py-12 space-y-8 flex flex-col justify-start">
        <div className="text-center space-y-4">
          ...
          >
            <ShieldCheck className="h-10 w-10 text-[#064e3b]" />
          </motion.div>
          <div className="space-y-1">
            <h1 className="text-xl md:text-2xl font-black text-[#064e3b] tracking-tight">
              {displayTitle}
            </h1>
            <p className="text-[10px] uppercase font-extrabold tracking-widest text-[#10b981]">
              Privasi Santri & Pengurus Amanah
            </p>
          </div>
        </div>
        ...
        {hasContent ? (
          ...
          <div className="p-8 bg-teal-50 border border-teal-100 rounded-3xl text-center space-y-2">
            ...
            <p className="text-[11px] text-teal-700 font-sans font-semibold uppercase tracking-wide">
              Kebijakan privasi belum dibuat oleh Admin utama.
            </p>
          </div>
        )}
      </div>
